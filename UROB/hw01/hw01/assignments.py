...
def dkt(omega: list, rho: list, base: np.ndarray) -> np.ndarray:
    ...
    for k in range(n):
        ...
        # -------------------------------------------------
        # START OF YOUR CODE
        phi = phi + Tensor(omega[k])
        # print(phi)
        pos_x = pos_x + phi.sin() * Tensor(rho[k])
        # print(pos_x)
        pos_y = pos_y + phi.cos() * Tensor(rho[k])

        # -------------------------------------------------

        # Add joint position to list of joint positions
        x.append(pos_x.stack(pos_y))
...
def dkt_vectorized(omega: list, rho: list, base: np.ndarray) -> np.ndarray:
    ...
    omega = np.concatenate((base[0].flatten(), to_numpy(omega).flatten())).flatten()
    rho = np.concatenate((base[1].flatten(), to_numpy(rho).flatten()))
...
def manipulator_loss(X: np.ndarray, x_goal: np.ndarray) -> Tensor:
    ...
    # -------------------------------------------------
    # START OF YOUR CODE
    loss = (Tensor(x_goal).T() - Tensor(X)).sum() ** 2
